...
describe('jev', () => {
  ...
  test('the gateway is never asked for a noul, which it refuses', () => {
    ...
    for (const t of types) assert.ok(['choice', 'score', 'boolean'].includes(t))
  })
  ...
  test('a failure is named, so the session can say why it went unrouted', async () => {
    ...
    const refused = await askJev({
      ...
      fetch: async () => ({
        ok: false,
        status: 403,
        text: JSON.stringify({ error: { type: 'customer_verification_required' } }),
      }),
    })
    assert.match(reasonOf(refused), /HTTP 403 \(customer_verification_required\)/)
  ...
  test('a call is timed, so the status report can show how slow Jev was', async () => {
    ...
    const got = await askJev({
      ...
      fetch: async () => {
        t += 640
        return answered({ answers: { tier: { type: 'choice', choice: 'opus' } } })
      },
    })
...
describe('label', () => {
  ...
  test('an unconfident pick is marked', () => {
    assert.equal(labelOf({ ...decision, confidence: 0.2 }, true), 'jev → opus·high?')
  })
  ...
  test('the label joins the engine’s own modes without duplicating', () => {
    assert.deepEqual(withLabel(['plan mode'], 'jev off'), ['plan mode', 'jev off'])
